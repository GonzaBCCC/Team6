week 4:
	...
	How to use for the Task 1:
		...
		the variables pathTrain, pathTest and GlobalPath, contain the path of the train dataset, test dataset and current folder of the 
		files respectively.
	...
	How to use for the Task 2:
		...
		The main files are M1ProjectGroup06_task2_week4(use to make the test) and M1ProjectGroup06_task2_week4_test(used to compute the images of test). In the main it expect to path_templates to indicate 
		...
		needs to be the  binary mask of the test split (validation) of the previous weeks. In the split_by_shape expects the path to train, the path to the templates and if we want an artificial mask(previous parameter) or a mask calculated.
		In  evalmodel_with_windows(directory,windowSet) we compute all the necessary to make the resulting mask.The first param is the directory where the binary masks are and the second one is the struct with our model. 
		...
		This function will have the bbox candidates for this images as output. 
		Then CandidateGenerationPixel_Color_window will select only the segments of the image that appear in the bboxs and generate the resulting mask for the given images then we compute the stadistics.
